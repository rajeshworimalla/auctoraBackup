...
-- Create indexes
CREATE INDEX idx_artwork_owner_id
ON "Artwork"(owner_id);
CREATE INDEX idx_artwork_category
ON "Artwork"(category);
CREATE INDEX idx_artwork_price
ON "Artwork"(price);
...
ADD CONSTRAINT wishlist_artwork_id_fkey FOREIGN KEY (artwork_id) REFERENCES "Artwork"(artwork_id)
ON DELETE CASCADE;

ALTER TABLE auctions
ADD CONSTRAINT auctions_artwork_id_fkey FOREIGN KEY (artwork_id) REFERENCES "Artwork"(artwork_id)
ON DELETE CASCADE;

ALTER TABLE cart_items
ADD CONSTRAINT cart_items_artwork_id_fkey FOREIGN KEY (artwork_id) REFERENCES "Artwork"(artwork_id)
ON DELETE CASCADE;
...
WITH CHECK (
  EXISTS (
    ...
    FROM
      "Artwork"
    WHERE
      artwork_id = auctions.artwork_id
...
WITH CHECK (
  ...
  AND NOT EXISTS (
    ...
    FROM
      "Artwork" a
      JOIN auctions auc
    ON a.artwork_id = auc.artwork_id
    WHERE
      auc.auction_id = trendingbids.auction_id
