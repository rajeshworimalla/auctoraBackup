-- First create the Artwork table
...
-- Insert placeholder records for artworks referenced in auctions and cart_items
WITH missing_artworks AS (
    SELECT DISTINCT artwork_id 
    FROM (
        SELECT artwork_id FROM auctions
        UNION
        SELECT artwork_id FROM cart_items
        UNION
        SELECT artwork_id FROM wishlist
    ) all_artwork_refs
